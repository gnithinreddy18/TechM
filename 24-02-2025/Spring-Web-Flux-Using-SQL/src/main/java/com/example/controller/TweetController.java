...
import com.example.exception.TextNotFoundException;
...
public class TweetController {
    ...
    @GetMapping("/tweets")
    public Flux<TweetInfo> getAll() {
        return repo.findAll();
    }

    @GetMapping("/tweets/{id}")
    public Mono<ResponseEntity<TweetInfo>> getTweetById(@PathVariable(value = "id") Integer tweetId) {
        return repo.findById(tweetId)
                .map(ResponseEntity::ok)
                .switchIfEmpty(Mono.error(new TextNotFoundException(tweetId))); // Throw exception if not found
    }
    ...
    public Mono<ResponseEntity<TweetInfo>> updateTweet(@PathVariable(value = "id") Integer tweetId,
    ...
    public Mono<ResponseEntity<Object>> deleteTweet(@PathVariable(value = "id") Integer tweetId) {
        return repo.findById(tweetId)
    ...
    public ResponseEntity<ErrorResponse> handleTweetNotFoundException(TextNotFoundException ex) {
